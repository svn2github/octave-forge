...
create_expat() {
  vsrv=http://ftp.de.debian.org/debian/pool/main/e/expat
  vxpt=(expat_2.0.1.orig.tar.gz ee8b492592568805593f81f8cdf2a04c)

  file_curl_md5 $vsrv/${vxpt[0]} ./work/${vxpt[0]} ${vxpt[1]}
  tar -C ./work -vxzf ./work/${vxpt[0]} >> $vmsg 2>&1
  cd ./work/expat-2.0.1
  conf_make_inst "$vcnf --enable-shared --disable-static"
  cd $vpwd
}
...
create_fontconfig() {
  vsrv=http://ftp.de.debian.org/debian/pool/main/f/fontconfig
  vfcg=(fontconfig_2.6.0.orig.tar.gz ab54ec1d4ddd836313fdbc0cd5299d6d)

  file_curl_md5 $vsrv/${vfcg[0]} ./work/${vfcg[0]} ${vfcg[1]}
  tar -C ./work -vxzf ./work/${vfcg[0]} >> $vmsg 2>&1
  cd ./work/fontconfig-2.6.0
#  vsav=$vbsh
#  vbsh="$vbsh LIBXML2_CFLAGS=-I/usr/include/libxml2 LIBXML2_LIBS=\"-L/usr/lib -lxml2\""
  conf_make_inst "$vcnf --enable-shared --disable-static"
#  vbsh=$vsav
  cd $vpwd
}
...
create_libxml2() {
  vsrv=http://ftp.de.debian.org/debian/pool/main/libx/libxml2
  vxml=(libxml2_2.6.32.dfsg.orig.tar.gz bb11c95674e775b791dab2d15e630fa4)

  file_curl_md5 $vsrv/${vxml[0]} ./work/${vxml[0]} ${vxml[1]}
  tar -C ./work -vxzf ./work/${vxml[0]} >> $vmsg 2>&1
  cd ./work/libxml2-2.6.32
  conf_make_inst "$vcnf --disable-static --without-python"
  cd $vpwd
}
...
create_octave() {
  ...
  voct=(octave-3.2.0.tar.gz fa15910c7382d1e13fe8e360edac1863)
  vopc=(./patches/octave-3.2.0.macosx.diff.gz 649d2527817a806cd6950d2658f71adb)
...
create_pkg-config() {
  vsrv=http://ftp.de.debian.org/debian/pool/main/p/pkg-config
  vpkc=(pkg-config_0.22.orig.tar.gz fd5c547e9d66ba49bc735ccb8c791f2a)

  file_curl_md5 $vsrv/${vpkc[0]} ./work/${vpkc[0]} ${vpkc[1]}
  tar -C ./work -vxzf ./work/${vpkc[0]} >> $vmsg 2>&1
  cd ./work/pkg-config-0.22
  conf_make_inst "$vcnf --enable-static --disable-shared"
  cd $vpwd
}

create_png() {
  vsrv=http://ftp.de.debian.org/debian/pool/main/libp/libpng
  vpng=(libpng_1.2.36.orig.tar.gz 4779cd011409ea9b591e11638781f0bf)

  file_curl_md5 $vsrv/${vpng[0]} ./work/${vpng[0]} ${vpng[1]}
  tar -C ./work -vxzf ./work/${vpng[0]} >> $vmsg 2>&1
  cd ./work/libpng-1.2.36
...
clear_dirs() {
  ...
  rm -rvf \
    ...
    zlib-1.2.3.3.dfsg libpng-1.2.36
  rm -rvf \
    octave-3.2.0 fontconfig-2.6.0 expat-2.0.1 pkg-config-0.22
  cd $vpwd
}
...
case $2 in
  ...
  -hdf5)           create_hdf5;;
  -jpg)            create_jpg;;
#  -libxml2)        create_libxml2;;
  -octave)         create_octave;;
# -openjpeg)       create_openjpeg;;
  -pkg-config)     create_pkg-config;;
  -png)            create_png;;
  -pcre)           create_pcre;;
  -qhull)          create_qhull;;
