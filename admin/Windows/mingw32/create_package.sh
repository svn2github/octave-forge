...
( cd ${PACKAGE_ROOT} && ${TAR} cjvf ${TOPDIR}/${PKGFILE}.tar.bz2 . );
...
CF=7
"${SEVENZIP}" a -t7z ${TOPDIR}/${PKGFILE}.exe ${PACKAGE_ROOT}/* -mx${CF} -sfx7z.sfx
"${SEVENZIP}" a -t7z ${TOPDIR}/${PKGFILE}_mingw32-msys.exe ${PACKAGE_ROOT}{/mingw32,/msys} -mx${CF} -sfx7z.sfx
"${SEVENZIP}" a -t7z ${TOPDIR}/${PKGFILE}_wo-compliler.exe ${PACKAGE_ROOT}{/bin,/doc,/include,/info,/lib,/libexec,/man,/share} -mx${CF} -sfx7z.sfx
