...
"@seealso{gf,primpoly}")
{
  ...
  for (octave_idx_type i=0; i<a.rows(); i++) {
    for (octave_idx_type j=0; j<a.columns(); j++) {
      if (((octave_idx_type)a(i,j) != 0) && ((octave_idx_type)a(i,j) != 1)) {
	poly = false;
	break;
      }
    }
  ...
  if (poly) {
    ...
    for (octave_idx_type i=0; i<a.rows(); i++) {
      octave_idx_type tmp = (octave_idx_type)a(i,0);
      for (octave_idx_type j=1; j<a.columns(); j++)
	tmp = (tmp << 1) | (octave_idx_type)a(i,j);
	...
	b(i,j) = do_isprimitive((octave_idx_type)a(i,j), m);
      }
    }
    retval = octave_value (b);
  }
