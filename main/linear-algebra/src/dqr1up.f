...
c 
      subroutine dqr1up(m,n,Q,R,u,v)
c purpose:      updates a QR factorization after rank-1 modification
c               i.e., given a m-by-m unitary Q and m-by-n upper 
...
c NOTE:         uses DLARTG rather than DROTG
      integer m,n
      double precision Q(m,m),R(m,n),u(m),v(n)
      double precision c,s,w,w1,rr,ddot
      external dlartg,drot,daxpy,ddot
      integer i
      if (m <= 0 .or. n <= 0) return
c form each element of w = Q'*u when necessary.
      w = ddot(m,Q(1,m),1,u,1)
      do i = m-1,1,-1
        w1 = w
...
c apply rotation to rows of R        
        if (i <= n) then
          call drot(n+1-i,R(i,i),m,R(i+1,i),m,c,s)
        end if
...
c apply rotation to rows of R
        if (i < n) then
          call drot(n-i,R(i,i+1),m,R(i+1,i+1),m,c,s)
        end if
c apply rotation to columns of Q
        call drot(m,Q(1,i),1,Q(1,i+1),1,c,s)
      end do
