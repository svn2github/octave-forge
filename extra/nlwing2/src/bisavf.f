...
c 
      subroutine bisavf(m,n,CP,X,dir,vi)
...
c
      integer m,n
      double precision CP(m,3),X(n,3),dir(3),vi(m,n,3)
      integer i, j
      ...
      do j = 1,n
        ...
        do i = 1,m
          ...
          RXn = sqrt(sum(RX**2))
          Rf = pi4i / (RXn*(RXn + dot))
          vi(i,j,1) = Rf*(RX(2)*dir(3) - RX(3)*dir(2))
          vi(i,j,2) = Rf*(RX(3)*dir(1) - RX(1)*dir(3))
          vi(i,j,3) = Rf*(RX(1)*dir(2) - RX(2)*dir(1))
        end do
      end do
      end subroutine
