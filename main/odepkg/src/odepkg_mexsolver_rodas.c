...
/* To manually compile this file with debug messages
     ...
     odepkg_mexsolver_rodas.c odepkgmex.c odepkgext.c hairer/rodas.f \
     hairer/decsol.f hairer/dc_decsol.f

   or

     mex -v -D__ODEPKGDEBUG__ odepkg_mexsolver_rodas.c odepkgmex.c \
     odepkgext.c hairer/rodas.f hairer/decsol.f hairer/dc_decsol.f

   a function check can be done via

     octave --quiet --eval "tic; A = odeset ('Jacobian', @jac, 'Stats', 'on'); \
       B = odepkg_mexsolver_rodas (@fun, [0 2], [2 0], A, 1e-6); toc"
*/
...
extern double F77_FUNC (contro, CONTRO) (int *I, double *S, double *CONT, int *LRC);

extern void F77_FUNC (fdfx, FDFX) (GCC_ATTR_UNUSED int *N, 
  GCC_ATTR_UNUSED double *X, GCC_ATTR_UNUSED double *Y, 
  GCC_ATTR_UNUSED double *FX, GCC_ATTR_UNUSED double *RPAR, 
  GCC_ATTR_UNUSED int *IPAR) {
  mexPrintf ("This function needs to be defined but is never be called!");
}
    ...
    if (vref > 0) { /* If the Refine option is > 0 */
      ...
      for (vcnt = 0; vcnt < vref; vcnt++) {
        ...
        for (vnum = 1; vnum <= *N; vnum++)
          vdob[vnum-1] = F77_FUNC (contro, CONTRO) (&vnum, &vdbt, CONT, LRC);
...
void mexFunction (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) { 
  ...
  double  H = 0.0;      /* Initial step size guess, rodas will set this */
  ...
  if (nrhs == 0) { /* Check number and types of all input arguments */
    ...
    mexUsgMsgTxt ("[t, y] = odepkg_mexsolver_rodas (fun, slot, init, varargin)");
  }
  ...
  /* IWORK[00] = 0; */    /* Maximal number of allowed steps */
  /* IWORK[01] = 7; */    /* Switch for the choice of the coefficients */
  /* IWORK[02] = 0; */    /* Switch for step size strategy */
  /* IWORK[08] = 0; */    /* Value for the M1 variable */
  /* IWORK[09] = 0; */    /* Value for the M2 variable */

  /* FORTRAN SUBROUTINE RODAS(N,FCN,X,Y,XEND,H,RTOL,ATOL,ITOL,JAC,IJAC,MLJAC,MUJAC, */
  ...
  vcnt = 1; /* Tell the solver that F(X,Y) may depend on X */
  vint = 0; /* Tell the solver to compute derivatives internally */
  F77_FUNC (rodas, RODAS) (&N, &F77_FUNC (ffcn, FFCN), &vcnt, &SLOT[0],
    INIT, &SLOT[1], &H, RTOL, ATOL, &ITOL,
    &F77_FUNC (fjac, FJAC), &IJAC, &MLJAC, &MUJAC, F77_FUNC (fdfx, FDFX), &vint,
  ...
  if (nlhs == 1) { /* Handle the PLHS array (1 output argument) */
    ...
    mxSetFieldByNumber (plhs[0], 0, vnum, mxCreateString ("oders"));
