...
%%    default decimal delimiter is '.' char(46), sometimes (e.g in 
...
elseif ischar(s),
        ...
        while ix <= sl;
                ...
                if ~isempty(te),
                        ...
                        flag = 0;
                        while any(abs(s(ix))==[cdelim,rdelim]) & (ix < sl),
                                flag = flag | any(abs(s(ix))==rdelim);
                                ix = ix + 1;
                        end;
...
for k2 = 1:nc,
        ...
        epos = find((t==abs('e')) | (t==abs('E')));	%% positon of E 
	if (length(epos)>1), 			%% if more than one E is found 
		status(k1,k2) = -1;		%% return error code
                ...
                elseif all(sum(t(1:epos-1)==ddelim)<[2,epos-1]), 	% at most one dot, and at least one digit is needed 
                        p = 0;
                        while l < epos,
                                if any(t(l)=='0123456789');
                                        g = g*10 + t(l) - '0';        
