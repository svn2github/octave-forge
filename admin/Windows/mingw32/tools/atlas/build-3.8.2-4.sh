...
DIRS=`ls -1 | sed -ne "/^ARCH_\([A-Za-z0-9]\+\)_atlas-${VER}_gcc-4./p"`
ARCHS=`ls -1 | sed -ne "s/^\(ARCH_[A-Za-z0-9]\+\)_atlas-${VER}_gcc-4./\1/p"`
...
build()
{
   ...
   for a in $DIRS; do
	...
	-e "s+@SRCDIR@+${TOPDIR}/$a+" \
	-e "s+@TOPDIR@+${TOPDIR}+" \
	-e "s+@GCC_VER@+${GCC_VERSION}+" \
	-e "s+@GCC_SYS@+${GCC_SYSTEM}+" \
	-e "s+@REL@+${REL}+" \
	${TOPDIR}/makefile-4.in > "${BDIR}/makefile" 
      make_common -C $BDIR shlibs
   done
}
