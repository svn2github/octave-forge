...
2001-08-26 "Boleslaw Ciesielski" <bolek@curl.com>
* Allow parameters to be passed through the call to fsolve() to the
  user-supplied function.  Don't place an upper limit on the number
  of arguments.
...
static octave_value_list fsolve_extra_args;
...
Any optional arguments specified in the call to @code{fsolve}\n\
...
@end deftypefn")
{
  ...
  if (nargin >= 2 && nargout < 4)
	...
	FSOLVE_ABORT1 ("expecting vector as second argument");
      ...
      int nextra = nargin - 2;
      fsolve_extra_args.resize (nextra);
