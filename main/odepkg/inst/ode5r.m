...
function [varargout] = ode5r (varargin)
  if (exist ('odepkg_mexsolver_radau5') != 3)
    error ('Mex-function "odepkg_mexsolver_radau5" is not installed');
  else
    [varargout{1:nargout}] = odepkg_mexsolver_radau5 (varargin{:});
    end
  end
...
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    vsol = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0]);
%!  end 
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    [vx, vy] = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0]);
%!  end
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    [vx, vy, va, vb, vc] = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0]);
%!  end
%#
...
%!    vsol = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0], A);
%!  end
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    A = odeset ('MaxStep', 0.1, 'RelTol', 1e-2, 'AbsTol', 1e-3);
%!    [vx, vy] = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0], A);
%!  end
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    A = odeset ('MaxStep', 0.1, 'RelTol', 1e-2, 'AbsTol', 1e-3);
%!    [vx, vy, va, vb, vc] = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0], A);
%!  end
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    A = odeset ('MaxStep', 0.1, 'RelTol', 1e-2, 'AbsTol', 1e-3);
%!    vsol = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0], A, 1.2);
%!  end
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    A = odeset ('MaxStep', 0.1, 'RelTol', 1e-2, 'AbsTol', 1e-3);
%!    [vx, vy] = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0], A, 1.2);
%!  end
%!test
%!  if (!strcmp (which ("odepkg_mexsolver_radau5"), ""))
%!    A = odeset ('MaxStep', 0.1, 'RelTol', 1e-2, 'AbsTol', 1e-3);
%!    [vx, vy, va, vb, vc] = ode5r (@odepkg_equations_vanderpol, [0 2], [2 0], A, 1.2);
%!  end
