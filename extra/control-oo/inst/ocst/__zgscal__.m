...
function x = __zgscal__ (a, b, c, d, z, n, m, p)
  ...
  F = eye (nmp);
  for kk = 1:nmp
    F(1:nmp,kk) = __zgfmul__ (a, b, c, d, F(:,kk));
  endfor
  ...
  while ((k < 2*len_x && xnorm > 0.5 && rnorm > fnorm) || k == 0)
    ...
    zk1= __zgfslv__ (n, m, p, rk1);

    ## Generalized CG iteration
    ## gamk1 = (zk1'*F_d*zk1)/(zk1'*F*zk1);
    ztMz1 = zk1'*rk1;
    gamk1 = ztMz1/(zk1'*__zgfmul__ (a, b, c, d, zk1));
  ...
  if (xnorm> 0.5 && rnorm > fnorm)
    ...
    Am = eye (nmp);
    for ii = 1:nmp
      Am(:,ii) = __zgfmul__ (a, b, c, d, Am(:,ii));
    endfor
