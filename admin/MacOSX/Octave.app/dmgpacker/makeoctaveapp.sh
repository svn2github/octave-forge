...
OCTBIN=${DEPSDIR}/bin/octave-${OCTAVEVERSION}

# This is a temporary directory that is used for copying Octave and
# its libraries (and remove other directories like 'man' etc.) that
# depend on Octave.app.
TEMPDIR=/tmp/TEMPDIR
...
OCEXTRAS=~/Savings/gnuplot-4.2.5-${TARGETPLATFORM}.dmg
...
if [ $# -ne 0 ]; then
  echo "Usage: makeoctaveapp.sh without any input argument"
  exit 1
else
...
echo "FIXME - COPYING *ALL* *a,*la LIBS AND *ALL* INCLUDES"
  ...
  evalfailexit "cp -R -P -vp ${OCDOCS} ${DMGDIR}/Doc"
  ...
  evalfailexit "hdiutil create -volname octave.app.dmg -fs HFS+ \
