...
int recv_class (MPI_Comm comm, octave_value &ov, int source, int mytag);        
...
#define __MAKE_CMPLX_TYPE_BRANCH__(TMPI,T1,A1)                          \
      ...
      else __MAKE_CMPLX_TYPE_BRANCH__(MPI_FLOAT,float,FloatComplexNDArray)
...
#define __MAKE_TYPE_BRANCH__(TMPI,T1,A1)                \
      if (TRCV == TMPI)                                 \
        {                                               \
          ...
          info = recv_vec(comm, LBNDA, nitem, TRCV,     \
                          source, tanktag[4]);          \
          if (info != MPI_SUCCESS)                      \
            return info;                                \
                                                        \
      ...
      else __MAKE_TYPE_BRANCH__(MPI_INT,bool,boolNDArray)
      else __MAKE_TYPE_BRANCH__(MPI_FLOAT,float,FloatNDArray)
      else __MAKE_TYPE_BRANCH__(MPI_BYTE,octave_int8,int8NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_SHORT,octave_int16,int16NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_INT,octave_int32,int32NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_LONG_LONG,octave_int64,int64NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_UNSIGNED_CHAR,octave_uint8,uint8NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_UNSIGNED_SHORT,octave_uint16,uint16NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_UNSIGNED,octave_uint32,uint32NDArray)
      else __MAKE_TYPE_BRANCH__(MPI_UNSIGNED_LONG_LONG,octave_uint64,uint64NDArray)
...
#define __MAKE_TYPE_BRANCH__(TMPI,T1,A1)                                \
  ...
  if (is_complex)
  ...
  else
    {
      ...
      else __MAKE_TYPE_BRANCH__(MPI_DOUBLE,double,SparseMatrix)
    }
...
Receive an MPI message containing an Octave variable packed into a contiguous memory chunk and extract its value.\n\
