...
URL="http://download.sourceforge.net/gnuplot/gnuplot-4.4.0-rc1.tar.gz"
...
mkdirs_pre() 
{ 
   for a in ${BUILDDIR_GUI} ${BUILDDIR_CONSOLE} ${BUILDDIR_DEBUG}; do
      if [ -e $a ]; then rm -rf $a; fi; 
   done
}
...
build() {
  ...
  MAKE_PARALLEL=""
  ( cd ${BUILDDIR_CONSOLE} && make_common -C src all )
...
install()
{
   ...
   ${CP} ${CP_FLAGS} ${BUILDDIR_CONSOLE}/src/wgnuplot.mnu ${BINARY_PATH}
   ${CP} ${CP_FLAGS} ${BUILDDIR_CONSOLE}/src/wgnuplot.hlp ${BINARY_PATH}
   ...
   mkdir -vp ${SHARE_PATH}/${PKG}/js
   ${CP} ${CP_FLAGS} ${SRCDIR}/term/js/* ${SHARE_PATH}/${PKG}/js
   ...
   ${CP} ${CP_FLAGS} ${BUILDDIR_CONSOLE}/src/gnuplot.pdf      ${DOC_PATH}/${PKG}
