...
sqrtm_from_schur(const ComplexMatrix& U, const ComplexMatrix& T)
{
  ...
  for (int j = 0; j < n; j++) R(j,j) = sqrt(T(j,j));
...
@seealso{expm logm funm}")
{
  ...
  if (arg.is_real_scalar ())
      ...
      if (arg.is_real_matrix ())
	{
	  ...
	  const SCHUR schur (A, std::string());
	  if (error_state) return retval;

	  // XXX FIXME XXX -- if the constructor
	  //    ComplexSchur(Matrix& U, Matrix& T) 
	  // is defined, use it instead
	  octave_value_list call;
	  call(0) = schur.unitary_matrix();
	  call(1) = schur.schur_matrix();
	  octave_value_list result = feval("rsf2csf", call, 2);
	  if (error_state) return retval;
	  
	  const ComplexMatrix U(result(0).complex_matrix_value());
	  const ComplexMatrix T(result(1).complex_matrix_value());
