...
/* To manually compile this file with debug messages
     ...
     odepkg_mexsolver_radau5.c odepkgmex.c odepkgext.c hairer/radau5.f \
     hairer/decsol.f hairer/dc_decsol.f

   or

     mex -v -D__ODEPKGDEBUG__ odepkg_mexsolver_radau5.c odepkgmex.c \
     odepkgext.c hairer/radau5.f hairer/decsol.f hairer/dc_decsol.f

   a function check can be done via

     octave --quiet --eval "tic; A = odeset ('Jacobian', @jac, 'Stats', 'on'); \
       B = odepkg_mexsolver_radau5 (@fun, [0 2], [2 0], A, 1e-6); toc"
*/
...
extern void F77_FUNC (radau5, RADAU5) (int *N, void *FCN, double *X, double *Y,
...
extern double F77_FUNC (contr5, CONTR5) (int *I, double *S, double *CONT, int *LRC);
    ...
    if (vref > 0) { /* If the Refine option is > 0 */
      ...
      for (vcnt = 0; vcnt < vref; vcnt++) {
        ...
        for (vnum = 1; vnum <= *N; vnum++)
          vdob[vnum-1] = F77_FUNC (contr5, CONTR5) (&vnum, &vdbt, CONT, LRC);
...
void mexFunction (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) { 
  ...
  double  H = 0.0;      /* Initial step size guess, radau5 will set this */
  ...
  if (nrhs == 0) { /* Check number and types of all input arguments */
    ...
    mexUsgMsgTxt ("[t, y] = odepkg_mexsolver_radau5 (fun, slot, init, varargin)");
  }
  ...
  if (!mxIsEqual (mxGetField (vtmp, 0, "Mass"), mxGetField (vtem, 0, "Mass"))) {
    ...
    /* BugFix for the radau5 solver - if no Jacobian but a mass matrix is given */
  ...
  F77_FUNC (radau5, RADAU5) (&N, &F77_FUNC (ffcn, FFCN), &SLOT[0],
  ...
  if (nlhs == 1) { /* Handle the PLHS array (1 output argument) */
    ...
    mxSetFieldByNumber (plhs[0], 0, vnum, mxCreateString ("ode5r"));
