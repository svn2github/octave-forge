...
function [A, B, C, r, x, contacts] = vbcs (t, xin = 0)
  persistent input Alin Blin Clin A B C contacts r
  persistent Anl Bnl Cnl 
  % build the constant part of the system 
  if (isempty (Alin))
  ...
  if (isscalar (xin) || isempty(xin))
    xin = zeros ([input.totextvar + input.totintvar, 1]);
  endif
  % build the time-dependent part of the system
  [Anl, Bnl, Cnl] = asm_build_system (input, xin, t);
  % sum the two parts
  [A, B, C, x] = deal (Alin + Anl, Blin + Bnl, Clin + Cnl, xin);
endfunction
