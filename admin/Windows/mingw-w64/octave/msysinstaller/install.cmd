...
rem
call :msysinstall msysCORE-1.0.16-1-msys-1.0.16-bin.tar.lzma
call :msysinstall msysCORE-1.0.16-1-msys-1.0.16-ext.tar.lzma
...
call :msysinstall coreutils-5.97-3-msys-1.0.13-ext.tar.lzma
call :msysinstall bash-3.1.17-3-msys-1.0.13-bin.tar.lzma
call :msysinstall make-3.81-3-msys-1.0.13-bin.tar.lzma

call :msysinstall mintty-0.9.5-1-msys-1.0.16-bin.tar.lzma
...
call :msysinstall perl-5.6.1_2-2-msys-1.0.13-bin.tar.lzma

mkdir "%DST%\mingw"
call :msysinstall automake1.11-1.11.1-1-mingw32-bin.tar.lzma   \mingw
call :msysinstall automake-4-1-mingw32-bin.tar.lzma            \mingw
call :msysinstall libtool-2.4-1-mingw32-bin.tar.lzma           \mingw
call :msysinstall autoconf2.5-2.67-1-mingw32-bin.tar.lzma      \mingw
call :msysinstall autoconf-9-1-mingw32-bin.tar.lzma            \mingw

call :install junction.zip  http://download.sysinternals.com/Files/junction.zip \mingw\bin
...
echo wget.exe...
COPY /Y %SRCDIR%\wget.exe "%DST%\mingw\bin\wget.exe" >NUL

call :install gperf-3.0.1-bin.zip    http://downloads.sourceforge.net/gnuwin32/gperf-3.0.1-bin.zip \mingw\bin "--strip-components=1 bin/gperf.exe"

rem add /build mount to svn root of octave/mingw64
FOR /F "usebackq" %%a in (`cd ..\.. ^&^& cd ^| "%DST%\bin\sed.exe" -e ""s#\\\\#/#g""`) DO SET RR=%%a
...
:msysinstall
call :install "%~1" "http://downloads.sourceforge.net/mingw/%~1" "%~2"
goto :EOF
