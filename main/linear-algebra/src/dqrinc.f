...
c NOTE:         uses DLARTG rather than DROTG
      integer m,n,j
      double precision Q(m,m),R(m,n),x(m)
      double precision c,s,w,w1,rr,ddot
      external dlartg,drot,ddot,dgemv
      integer i
      if (m <= 0 .or. n <= 0) return
      if (j < 1 .or. j > n) then
        call xerbla('DQRINC',5)
      end if
c form each element of w = Q'*x when necessary.
      w = ddot(m,Q(1,m),1,x,1)
      do i = m-1,j,-1
        w1 = w
        w = ddot(m,Q(1,i),1,x,1)
...
c compute the rest of the column      
      call dgemv('T',m,j-1,1d0,Q,m,x,1,0d0,R(1,j),1)
      end subroutine
