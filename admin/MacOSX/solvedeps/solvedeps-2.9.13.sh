...
OCTAVEPACK=ftp://ftp.octave.org/pub/octave/bleeding-edge/octave-2.9.13.tar.gz
OCTAVEDIFF=./octave-2.9.13.diff
...
create_f2c () {
  ...
  echo "solvedeps-2.9.13.sh: Creating installation directories ..."
  evalfailexit "install -d ${INSTDIR}{,/bin,/lib,/include}"

  echo "solvedeps-2.9.13.sh: Applying the patch ${vf2cdiff} ..."
  evalfailexit "patch -p0 < ${vf2cdiff}"

  echo "solvedeps-2.9.13.sh: Making ${vf2cfile}/src ..."
  ...
  echo "solvedeps-2.9.13.sh: Making ${vf2cfile}/libf2c ..."
  cd ../libf2c
  evalfailexit "${MAKE}"
...
create_sparse() {
  ...
  echo "solvedeps-2.9.13.sh: Checking for target directories ..."
  evalfailexit "install -d ${INSTDIR}{,/bin,/lib,/include}"
  evalfailexit "install -c -S UFconfig/UFconfig.h ${INSTDIR}/include"
 
  echo "solvedeps-2.9.13.sh: Installing UMFPACK files in ${INSTDIR}"
  ...
  echo "solvedeps-2.9.13.sh: Installing AMD files in ${INSTDIR}"
  ...
  echo "solvedeps-2.9.13.sh: Installing CAMD files in ${INSTDIR}"
  ...
  echo "solvedeps-2.9.13.sh: Installing COLAMD files in ${INSTDIR}"
  ...
  echo "solvedeps-2.9.13.sh: Installing CCOLAMD files in ${INSTDIR}"
  ...
  echo "solvedeps-2.9.13.sh: Installing CXSPARSE files in ${INSTDIR}"
  ...
  echo "solvedeps-2.9.13.sh: Installing CHOLMOD files in ${INSTDIR}"
...
create_octave() {
  ...
  echo "solvedeps-2.9.13.sh: Applying patch ${voctavediff} ..."
  ...
  echo "solvedeps-2.9.13.sh: Removing special strings in octave-bug-${voctversion} ..."
  sed "s:${CFLAGS}::g;s:${LDFLAGS}::g;s:${ARCH}::g" \
...
if [ $# -ne 2 ]; then
    echo "Usage: solvedeps-2.9.13.sh LIBRARY ARCHITECTURE"
    exit 1
else
  case "${2}" in
    --ppc)
    ...
    *)
      echo "solvedeps-2.9.13.sh: Unknown input argument ${2}"
      exit 1
      ;;
  ...
  case "${1}" in
    ...
    --all)
      ...
      ./solvedeps-2.9.13.sh --f2c ${2}
      ./solvedeps-2.9.13.sh --readline ${2}
      ./solvedeps-2.9.13.sh --pcre ${2}
      ./solvedeps-2.9.13.sh --zlib ${2}
      ./solvedeps-2.9.13.sh --hdf5 ${2}
      ./solvedeps-2.9.13.sh --fftw ${2}
      ./solvedeps-2.9.13.sh --curl ${2}
      ./solvedeps-2.9.13.sh --glpk ${2}
      ./solvedeps-2.9.13.sh --sparse ${2}
      ./solvedeps-2.9.13.sh --octave ${2}
      ;;

    *)
      echo "solvedeps-2.9.13.sh: Unknown input argument ${1}"
      exit 1
      ;;
