...
%#
%# @deftypefnx {Function} {@var{[vsol]} =} ode54 (@var{@@vfun}, @var{vslot}, @var{vinit}, @var{[vopt]}, @var{[vP1, vP2, @dots{}]})
...
%#
%# @deftypefnx {Function} {@var{[vt, vy, [vte, vye, vie]]} =} ode54 (@var{@@vfun}, @var{vslot}, @var{vinit}, @var{[vopt]}, @var{[vP1, vP2, @dots{}]})
...
function [varargout] = ode54 (vfun, vslot, vinit, varargin)
  ...
  if (nargin == 0) %# Check number and types of all input arguments
    ...
    error (vmsg);
  elseif (nargin < 3)
    vmsg = sprintf ('[vt, vy] = ode54 (vfun, vslot, vinit, varargin)\n');
    usage (vmsg);
  ...
  %# Starting initialisation of ode54
  ...
  vcntloop = 2; vu = vinit; vk = vu' * zeros (1,7);
...
%! [vx, vy] = ode54 (@odepkg_equations_secondorderlag, [0 2.5], [0 0], ...
%!    A, 5, 2, 0.02, 0.1);
%!
