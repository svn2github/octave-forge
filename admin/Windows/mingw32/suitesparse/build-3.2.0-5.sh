...
URL="http://www.cise.ufl.edu/research/sparse/SuiteSparse/SuiteSparse-3.2.0.tar.gz"
...
{
  build_pre;
  ( cd ${BUILDDIR}/AMD/Lib      && make_common -f GNUmakefile );
  ( cd ${BUILDDIR}/CAMD/Lib     && make_common -f GNUmakefile );
  ( cd ${BUILDDIR}/CCOLAMD/Lib  && make_common );
  ( cd ${BUILDDIR}/COLAMD/Lib   && make_common );
  ( cd ${BUILDDIR}/CSparse/Lib  && make_common );
  ( cd ${BUILDDIR}/CXSparse/Lib && make_common );
  ( cd ${BUILDDIR}/CHOLMOD/Lib  && make_common );
  ( cd ${BUILDDIR}/UMFPACK/Lib  && make_common -f GNUmakefile );
  build_post;
}
...
install_lib_core()
{
   ${CP} ${CP_FLAGS} ${BUILDDIR}/$1/lib/$1.dll      $PREFIX/$BIN_DIR
   ${CP} ${CP_FLAGS} ${BUILDDIR}/$1/lib/lib$1.dll.a $PREFIX/$LIB_DIR
   ${CP} ${CP_FLAGS} ${BUILDDIR}/$1/lib/lib$1.a     $PREFIX/$STATICLIB_DIR
}

uninstall_lib_core()
{
   ${RM} ${RM_FLAGS} $PREFIX/$BIN_DIR/$1.dll 
   ${RM} ${RM_FLAGS} $PREFIX/$LIB_DIR/lib$1.dll.a 
   ${RM} ${RM_FLAGS} $PREFIX/$STATICLIB_DIR/lib$1.a 
}

install_header_core()
{
   for a in $2; do 
      ${CP} ${CP_FLAGS} ${SRCDIR}/$1/Include/$a $PREFIX/$INC_DIR
   done
}
...
install()
{
   ...
   ${CP} ${CP_FLAGS} ${SRCDIR}/UFconfig/UFconfig.h $PREFIX/$INC_DIR
   
   # Install license file
   ${CP} ${CP_FLAGS} ${SRCDIR}/AMD/doc/lesser.txt             $PREFIX/$LIC_DIR/$PKG/LGPL.txt
   ${CP} ${CP_FLAGS} ${SRCDIR}/AMD/doc/License                $PREFIX/$LIC_DIR/$PKG/LICENSE.AMD
   ${CP} ${CP_FLAGS} ${SRCDIR}/CAMD/doc/License               $PREFIX/$LIC_DIR/$PKG/LICENSE.CAMD
   ${CP} ${CP_FLAGS} ${SRCDIR}/CCOLAMD/Readme.txt             $PREFIX/$LIC_DIR/$PKG/Readme.CCOLAMD
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Check/License.txt      $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Check
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Cholesky/License.txt   $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Cholesky
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Core/License.txt       $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Core
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Demo/License.txt       $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Demo
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/MatrixOps/License.txt  $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.MatrixOps
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Modify/License.txt     $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Modify
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Partition/License.txt  $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Partition
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Supernodal/License.txt $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Supernodal
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Tcov/License.txt       $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Tcov
   ${CP} ${CP_FLAGS} ${SRCDIR}/CHOLMOD/Valgrind/License.txt   $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Valgrind
   ${CP} ${CP_FLAGS} ${SRCDIR}/COLAMD/Readme.txt              $PREFIX/$LIC_DIR/$PKG/Readme.COLAMD
   ${CP} ${CP_FLAGS} ${SRCDIR}/CSparse/doc/License.txt        $PREFIX/$LIC_DIR/$PKG/LICENSE.CSparse
   ${CP} ${CP_FLAGS} ${SRCDIR}/CXSparse/doc/License.txt       $PREFIX/$LIC_DIR/$PKG/LICENSE.CXSparse
   ${CP} ${CP_FLAGS} ${SRCDIR}/UMFPACK/doc/License            $PREFIX/$LIC_DIR/$PKG/LICENSE.UMFPACK
...
{
   install;
   for a in umfpack cholmod amd camd colamd ccolamd cxsparse; do
      $STRIP $STRIP_FLAGS $PREFIX/$BIN_DIR/$a.dll
   done
}
...
uninstall()
{
   ...
   ${RM} ${RM_FLAGS} $PREFIX/$INC_DIR/UFconfig.h
   
   # Uninstall license file(s)
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LGPL.txt
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.AMD
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CAMD
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/Readme.CCOLAMD
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Check
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Cholesky
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Core
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Demo
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.MatrixOps
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Modify
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Partition
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Supernodal
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Tcov
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CHOLMOD.Valgrind
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/Readme.COLAMD
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CSparse
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.CXSparse
   ${RM} ${RM_FLAGS} $PREFIX/$LIC_DIR/$PKG/LICENSE.UMFPACK
