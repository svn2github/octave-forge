...
octave_idx_type odepkg_seulex_solfcn
  ...
  if (NR > 1) odepkg_auxiliary_solstore (vt, vy, vseulexoutsel, 1);
  ...
  if (!vseulexpltfun.is_empty ()) {
    ...
    IRTRN = (odepkg_auxiliary_evalplotfun
      (vseulexpltfun, vseulexoutsel, vt, vy, vseulexextarg, 1) ? 0 : -1);
    vseulexpltbrk = true;
  }
 ...
 ********************************************************************/
  ...
  odepkg_auxiliary_solstore (vtim, vsol, vseulexoutsel, 0);
  ...
  if (f77_exception_encountered)
    (*current_liboctave_error_handler) 
      ("Unrecoverable error in \"seulex\" core solver function");
  ...
  if (IDID < 0 &&
    ...
    error_with_id ("hugh:hugh", "error after solving %d", IDID);
    vretval(0) = 0.0;
    return (vretval);
  }
 ...
 *******************************************************************/
  ...
  odepkg_auxiliary_solstore (vtres, vyres, vseulexoutsel, 2);
...
%!  assert (B.y(end,:), [17.536442, -0.160655, 52.696175], 1e-3);
%+!test
%+!  A = odeset ('OutputFcn', @odeplot, 'OutputSel', [1 2], 'Refine', 2);
%+!  B = odesx (@flor, [0 0.2], [3 15 1], A);
%+!  assert (B.x(end), 0.2, 1e-12);
%-!  assert (B.y(end,:), [17.536442, -0.160655, 52.696175], 1e-3);
...
%!  B = odesx (@fbal, [0 3], [1 3], A);
%!  assert (B.ie, 1, 0);
%!  assert (B.xe, B.x(end), 0);
%!  assert (B.ye, B.y(end,:), 0);
%!test
%!  A = odeset ('Events', @feve, 'AbsTol', 1e-10, 'MaxStep', 1e-3);
%!  B = odesx (@fbal, [0 3], [1 3], A, 12, 13, 'KL');
%!  assert (B.ie, 1, 0);
%!  assert (B.xe, B.x(end), 0);
%!  assert (B.ye, B.y(end,:), 0);
%!test
%!  A = odeset ('Events', @feve, 'AbsTol', 1e-10);
%!  [t, y, xe, ye, ie] = odesx (@fbal, [0 3], [1 3], A);
%!  assert (ie, 1, 0);
%!  assert (xe, t(end), 0);
%!  assert (ye, y(end,:), 0);
%!test
%!  A = odeset ('Events', @feve, 'AbsTol', 1e-10);
%!  [t, y, xe, ye, ie] = odesx (@fbal, [0 3], [1 3], A, 12, 13, 'KL');
%!  assert (ie, 1, 0);
%!  assert (xe, t(end), 0);
%!  assert (ye, y(end,:), 0);
%!test
...
%+!  disp ('Calculating the Jacobian function');
%+!  vdy = [0, 1;
